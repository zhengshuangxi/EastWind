...
public class BLEListener : MonoBehaviour {
	...
	void Update () {
		...
		if(isBLEOpen)
			BLEbtn.transform.Find("Text").GetComponent<Text>().text = "Close BLE";
		else 
			BLEbtn.transform.Find("Text").GetComponent<Text>().text = "Open BLE";
		...
		root.transform.Find ("PicoVR/Head/Infor").GetComponent<Text> ().text = "";
		if(powerState == 5)
			root.transform.Find ("PicoVR/Head/Infor").GetComponent<Text> ().text += "powerState: On\n";
		else
			root.transform.Find ("PicoVR/Head/Infor").GetComponent<Text> ().text += "powerState: Off\n";

		root.transform.Find ("PicoVR/Head/Infor").GetComponent<Text> ().text += ("connect state: "+isConnected.ToString()+"\n");
		if(otaVersion != null)
			root.transform.Find ("PicoVR/Head/Infor").GetComponent<Text> ().text += ("otaVersion: "+otaVersion+"\n");
	...
	{
		root.transform.Find ("BLEListCanvas").gameObject.SetActive (true);
	...
	void BLEConnectedStateChangedEvent(object sender, System.EventArgs e)
	{
		...
		root.transform.Find ("BLEListCanvas").gameObject.SetActive (false);
		int isConnected = PicoVRManager.SDK.currentDevice.GetBLEConnectState();
		root.transform.Find ("PicoVR/Head/Infor").GetComponent<Text> ().text += isConnected.ToString();
	}


	public void ToggleBLEState(){
		string text  = BLEbtn.transform.Find("Text").GetComponent<Text>().text;
